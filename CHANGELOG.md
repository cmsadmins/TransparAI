...
- Footer notices: the page notice for labeled media, the chatbot notice in its footer variant and the
  ...
  source) instead of three stacked lines; new filter `transparai_footer_notices`. The chatbot footer
  line lost the `trai-chat-notice` class, which had styled it as a fixed corner box instead of a line.
- The page notice and the Schema.org output for labeled media no longer depend on the visible badge:
  with the badge off they still print, while the badge markup, script and alt-text addition stay off.
- Badge: the icon-only style and the mini variant on small images lost their short label in overlay
  mode (the em-based width cap collapsed to 0 at font-size 0 and clipped the "AI" chip away); both are
  exempt from the cap now.
- Settings: the "Appearance" row of the Visible badge tab shows the live badge preview from the setup
  card, including the size; the setup card preview uses the stored size instead of always medium.
- Defaults: the badge label also goes into the alt text of labeled images (`badge_alt_append`),
  the text note reaches RSS items (`feed_notice`, the in-page note skips feeds), the chatbot notice
...
- Report: `TransparAI_Meta::report()`, the print view and `GET /report` carry the compliance
  ...
  fourth limitation added. New CLI commands `wp transparai score`, `content` (list posts with an AI level, set or
  remove it), `assessment` (show, answer, tick), `systems` with `systems-declare`, `systems-undeclare`
  and `systems-visible`, and `report` (the audit record as JSON); `status --status=labeled`.
