...
A configurable badge marks labeled media in the front end. Overlay or a caption line, four positions, three sizes, dark, light, outline or icon-only, optional generator name, optional alt text note for screen readers. An optional start date limits the badge to media uploaded on or after that day, so you can introduce the labeling on an existing site without retroactively badging older content; earlier media stay labeled in the admin only. The badge is rendered server-side, so it survives page caching, and it works with the block editor (images, galleries, covers, featured images, inline images, video and audio), the classic editor, template images, text widgets, WooCommerce product and shop pages, Elementor free and Pro including Theme Builder, and WPBakery Page Builder, each image module tested one by one. In WooCommerce the badge follows a variation swap (a variation without its own image loses the parent's badge instead of wearing it wrongly), it appears again in the zoom and PhotoSwipe lightbox and in the core image lightbox, where every other plugin loses it, and it is kept out of WooCommerce e-mails, where positioned overlays only break. The plugin declares itself compatible with High-Performance Order Storage.
...
Every label, review decision, declaration and repair is recorded per file with its time, the state it replaced, what triggered it and the name of the editor who made it (the last fifty events per file), and a site log keeps settings changes, scans, sweeps and bulk actions. The whole library exports as a CSV audit list straight from the plugin page, the history included, and a print view adds a document hash over the facts (two reports of an unchanged site carry the same hash), the guidance basis and the stated limitations, ready for print-to-PDF. So the question "who declared this image AI-generated, and when" has an answer months later. A REST API under `transparai/v1` exposes the same rows, the per-file detail, every action and the report for headless setups and agency tooling; nothing in it is public. WP-CLI commands for scanning, labeling and auditing: `wp transparai scan`, `wp transparai status --format=csv` for the same export on the command line, `wp transparai verify-meta --repair` to check and fix the in-file metadata. Other plugins can label media through `do_action( 'transparai_mark_ai', $attachment_id, 'My Generator' )`, the detection rules are extensible via filters, translation setups are covered by a bundled WPML configuration, and uninstalling cleans up across every site of a multisite network when you ask it to.
