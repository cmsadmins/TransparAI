...
final class TransparAI_Systems {
	...
	/**
	 * Footer style: one line in the shared footer notice printed by
	 * TransparAI_Frontend, next to the media and chatbot lines.
	 *
	...
	public static function footer_notice( $lines ): array {
		...
		if ( 'footer' !== TransparAI_Options::get( 'systems_notice_style' ) ) {
			return $lines;
		}
		$systems = self::visible();
		if ( array() !== $systems ) {
			$lines['systems'] = self::notice_text( $systems );
		}
		return $lines;
	}

	/**
	 * Print the badge or banner style (wp_footer). Server-rendered, cache-safe;
	 * the footer style goes through footer_notice() instead.
	 */
