...
final class TransparAI_Dashboard {
	...
	public static function render_assessment(): void {
		...
		?>
		<section class="trai-card">
			<p><?php esc_html_e( 'The EU AI Act does not treat every website the same way: which transparency duties apply depends on how a site actually uses AI. A site without a chatbot has nothing to disclose about chatbots, a site that publishes AI-written articles does. This self-assessment finds out which of those uses apply to your site, so you only work on what matters here.', 'transparai' ); ?></p>
			<p><?php esc_html_e( 'Answer each of the six questions with Yes or No, as things stand today, not as they might be some day. If you are unsure, ask the people who produce the content for this site; the examples under each question show what counts. After saving, a table appears below with the articles that are likely to apply and a link to the plugin page that handles each one. Complete answers also count toward the readiness score on the dashboard and are printed in the compliance report.', 'transparai' ); ?></p>
			<p class="description"><?php esc_html_e( 'Stored on this site only, nothing is sent anywhere. You can change the answers at any time; whenever the site starts or stops using AI in one of these ways, come back and update them.', 'transparai' ); ?></p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
		...
		<?php if ( TransparAI_Compliance::assessment_complete() ) : ?>
			<section class="trai-card">
				...
				<?php endif; ?>
				<p class="description"><?php esc_html_e( 'Whether a paragraph of Article 50 addresses you as provider or as deployer depends on who built the system. This overview is a technical aid, not legal advice.', 'transparai' ); ?></p>
			</section>
		<?php else : ?>
			<section class="trai-card">
				<h2 class="trai-card-title"><?php esc_html_e( 'Your to-do list', 'transparai' ); ?></h2>
				<p class="description"><?php esc_html_e( 'This overview appears once all six questions are answered and saved. It lists the articles of the AI Act that are likely to apply to your site and the plugin page that handles each one.', 'transparai' ); ?></p>
			</section>
		<?php endif; ?>
		<?php
	...
	private static function render_literacy_card(): void {
		...
		<section class="trai-card" id="trai-literacy">
			...
			<p class="description"><?php esc_html_e( 'Article 4 asks providers and deployers to make sure the people who work with AI systems have sufficient AI literacy. Tick what your organisation has done; the checklist counts toward the readiness score and appears in the compliance report.', 'transparai' ); ?></p>
